...
此项目是一个依靠Cts框架,对原生Monkey进行改造后的产物,拥有以下新增功能:
...
# 执行和报告
默认配置文件在android-cts\tools\config\cts.xml，可自行修改
...
```
run cts --p 测试app的包名 --a 测试app的主activity
```
...
> 在命令行下输入```run cts --help-all``` 获取所有的可设置参数:
